...
def min_max(nums: list[float | int]) -> tuple[float | int, float | int]: #Function created
    if not nums:                                        # if not numbers
        raise ValueError ("List can not be empty")      #Show the ValueError: List can not be empty
...
def unique_sorted(nums: list[float | int]) -> list[float | int]: #Function created
    return sorted(set(nums))  #Return new ordenated list, with unique values (set "unduplicate" duplicated itens).
##################################################################################################  
...
def flatten(mat: list[list | tuple]) -> list: #creating function with returnig a unique list.
    flat_list=[]                                #New list, to "join" list and tuple
    for element in mat:                         #loop to pu all the lists/tuple in just one list.
        if type(element)==list or type(element)==tuple:   #verify fi it's a list/tuple
